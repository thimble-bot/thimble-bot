class Color {
  ...
  __getArrayValues(input, ...args) {
    if (input.constructor === Array) {
      if (input.length !== args.length) {
        throw new Error(this.__ERRORS.INVALID_INPUT_ARRAY_PROPS);
      }
    ...
    if (!this.__isValidObject(input, ...args)) {
      throw new Error(this.__ERRORS.INVALID_INPUT_OBJ_PROPS);
    }
  ...
  fromRGB(rgb) {
    switch (typeof rgb) {
      case 'string':
        ...
        if (rgb.length !== 3) {
          throw new Error(this.__ERRORS.INVALID_RGB);
        }
        this._rgb = {
          r: rgb[0],
          g: rgb[1],
      ...
      default:
        throw new TypeError(this.__ERRORS.INPUT_STRING_OR_OBJECT);
    }
  ...
  fromHex(hex) {
    ...
    if (!hex.match(/(^[0-9A-F]{6}$)|(^[0-9A-F]{3}$)/i)) {
      throw new Error(this.__ERRORS.INVALID_HEX);
    }
  ...
  fromHSL(hsl) {
    switch (typeof hsl) {
      case 'string':
        ...
        if (hsl.length !== 3) {
          throw new Error(this.__ERRORS.INVALID_HSL);
        }
  ...
  fromCMYK(cmyk) {
    switch (typeof cmyk) {
      case 'string':
        ...
        cmyk = cmyk.split(',').map(c => parseFloat(c.match(/\d+/)[0]));

        if (cmyk.length !== 4) {
          throw new Error(this.__ERRORS.INVALID_CMYK);
        }
